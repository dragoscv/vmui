...
import { Activity, Cpu, Home, Lightbulb, Minus, MonitorSmartphone, Square, X } from "lucide-react";
...
const PAGES: Array<{ id: PageId; label: string; icon: React.ReactNode; el: React.ReactNode }> = [
  ...
  { id: "servicii", label: "Servicii", icon: <Activity className="size-4" />, el: <Servicii /> },
...
export function App() {
  const [page, setPage] = React.useState<PageId>(() => (localStorage.getItem("vmui:page") as PageId) || "ambilight");
      ...
      <div className="bg-scene h-full grid grid-rows-[40px_1fr]">
        ...
        <div className="grid grid-cols-[212px_1fr] min-h-0">
          ...
          <main className="min-h-0 overflow-y-auto px-6 pb-8 pt-1">
            <div key={cur.id} className="page mx-auto max-w-[980px]">
              {cur.el}
            </div>
          </main>
        </div>
...
function Toasts() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-4 right-4 flex flex-col gap-2" aria-live="polite">
      {list.map((t) => (
