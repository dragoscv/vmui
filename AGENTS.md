...
- **i18n**: `next-intl` without URL locales. Locale = `vmui_locale` cookie → `Accept-Language` → `en` ([src/i18n/request.ts](src/i18n/request.ts)); switcher in the topbar. Every user-visible string goes through `useTranslations("ns")` (client) / `getTranslations("ns")` (server/actions) — **no hardcoded UI text, in either language**. Messages: `messages/{en,ro}.json` for `nav`/`common`/`home`; one folder per namespace `messages/<ns>/{en,ro}.json` (`homeCards`, `notify`, `nutrition`, `ambilight`, `vm`, `settings`, `auth`, `misc`) merged in [src/i18n/messages.ts](src/i18n/messages.ts). Keys are type-checked against the `en` files ([src/i18n/global.d.ts](src/i18n/global.d.ts)); add a key to **both** locales in the same edit. Server-side text sent to devices (notification cards, HA fallback) uses [src/lib/notify/i18n.ts](src/lib/notify/i18n.ts) with the device's stored `language`. `tsgo` caches `messages/*.json`: after editing them delete `tsconfig.tsbuildinfo` if a fresh key still reports as missing.
